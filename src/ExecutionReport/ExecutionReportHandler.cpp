...
void ExecutionReportHandler::toDB(const FIX44::ExecutionReport& execReport) const {
    ...
    try {
        ...
        if( this->sourceSystem == "TraderTools" ) {

            if( getAccountStr(execReport) == "Curve_LDN" ) /* zero-spread so would duplicate in order PL */
                return;

            unique_ptr<TraderToolsOrder> ttOrder(new TraderToolsOrder());
            ttOrder->setAccount(getAccountStr(execReport));
            ttOrder->setSymbol(getSymbolStr(execReport));
            ttOrder->setClOrdID(getClOrdIDStr(execReport));
            ttOrder->setCurrency(getCurrencyStr(execReport));
            ttOrder->setExecID(getExecIDStr(execReport));
            ttOrder->setOrderID(getOrderIDStr(execReport));
            ttOrder->setOrdStatus(getOrdStatusStr(execReport));
            ttOrder->setOrdType(getOrdTypeStr(execReport));
            ttOrder->setSide(getSideStr(execReport));
            ttOrder->setTimeInForce(getTimeInForceStr(execReport));
            ttOrder->setTransactTime(getTransactTimeStr(execReport));
            ttOrder->setSettlDate(getSettlDateStr(execReport));
            ttOrder->setListID(getListIDStr(execReport));
            ttOrder->setTradeDate(getTradeDateStr(execReport));
            ttOrder->setExecType(getExecTypeStr(execReport));
            ttOrder->setEffectiveTime(getEffectiveTimeStr(execReport));
            ttOrder->setNoContraBrokers(getNoContraBrokersStr(execReport));
            ttOrder->setSecondaryExecID(getSecondaryExecIDStr(execReport));
            ttOrder->setPartyID(getPartyIDStr(execReport));
            ttOrder->setContraBroker(getContraBrokerStr(execReport));
            ttOrder->setAvgPxStr(getAvgPxStr(execReport));
            ttOrder->setAvgPx(getAvgPx(execReport));
            ttOrder->setCumQty(getCumQty(execReport));
            ttOrder->setLastPx(getLastPx(execReport));
            ttOrder->setLastQty(getLastQty(execReport));
            ttOrder->setOrderQty(getOrderQty(execReport));
            ttOrder->setPrice(getPrice(execReport));
            ttOrder->setLeavesQty(getLeavesQty(execReport));

            ttOrder->processOrder();
